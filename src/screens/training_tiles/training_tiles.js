...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetTrainingsCount(query)
            .then(async (res) => {
                if (res.status) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetTrainingSingle({ TrainingsId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Trainings/edit/${id}`;
        if (type === 'view') _route = `/Trainings/view/${id}`;
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            ...
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Trainings/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
