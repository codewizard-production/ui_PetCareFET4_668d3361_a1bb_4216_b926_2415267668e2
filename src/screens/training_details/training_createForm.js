...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, trainingId, numfields;
        const mapItems = MapItems;

        let training = row['training'];

        numfields = Helper.GetAllNumberFields(training);
        if (numfields.length > 0) Helper.UpdateNumberFields(training, numfields);


        // Add Or Update Training
        rslt = await Support.AddOrUpdateTraining(training, dropDownOptions, []);
        if (rslt.status) {
            trainingId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Training and navigation entity if it is deos not exist
            let navItem = training.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "TrainingsId", value: parseInt(trainingId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateTraining(data, dropDownOptions);
        ...
        global.AlertPopup("success", "Training is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
