import TrainingJsonConfig from "config/training_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "training", expand: "", exclude: [], func: Support.AddOrUpdateTraining }
]

const FetchTrainingInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(TrainingJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of TrainingJsonConfig[elm]) {
...
const FetchTrainingDetails = async (trainingId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (trainingId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetTrainingSingle(trainingId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetTrainingSingle(trainingId);
            }
            
            if (rslt.status) {

                const training = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? training : training[source];
...
const Extract = async (trainingId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchTrainingInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(trainingId)) {
                    await FetchTrainingDetails(trainingId, enums).then(({ row, backRow }) => {
