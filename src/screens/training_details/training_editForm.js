...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], trainingId, training, numfields, mapItem;

        training = row['training'];
        trainingId = row['training'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) trainingId = rslt.id;
                            
                            const mapPropKey = training.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'training');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Training is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
